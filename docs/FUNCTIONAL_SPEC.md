...
| Module 10 — Analytics Engine | Aggregate insight for TRA, not surveillance | A minimal real event-collection backend now exists (Supabase — see the officer repo's `supabase-setup.sql`): `engine/telemetry.js` here sends one anonymized profile event per session plus one topic-only event per "Ask Anything" chat message (`classifyChatTopic` below), and the [biashara-guide-officer](https://github.com/hakrambuilders-cyber/biashara-guide-officer) repo reads back only aggregate-only database functions (never a raw row, never message text — enforced by Postgres RLS, not just UI convention), falling back to `engine/analytics.js`'s synthetic population (labeled "DEMO DATA") whenever there's no real activity yet. Region, notice type, and benefits-eligibility aren't part of the telemetry payload yet, so those breakdowns are synthetic-only for now. The officer repo is a genuinely separate project and deployment, desktop-oriented, behind a login gate (§9), not a screen inside the citizen SPA. This repo (the citizen app) contains no officer/dashboard code; the officer repo keeps hand-synced copies of `engine/core.js`, `engine/knowledge.js`, and `engine/analytics.js` |
