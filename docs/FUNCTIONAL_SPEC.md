...
| Module 10 — Analytics Engine | Aggregate insight for TRA, not surveillance | A minimal real event-collection backend now exists (Supabase — see the officer repo's `supabase-setup.sql`): `engine/telemetry.js` here sends one anonymized event per session, and the [biashara-guide-officer](https://github.com/hakrambuilders-cyber/biashara-guide-officer) repo reads back only aggregate-only database functions (never a raw row — enforced by Postgres RLS, not just UI convention), falling back to `engine/analytics.js`'s synthetic population (labeled "DEMO DATA") whenever there's no real activity yet. Region, notice type, chat topic, and benefits-eligibility aren't part of the telemetry payload yet, so those breakdowns are synthetic-only for now. The officer repo is a genuinely separate project and deployment, desktop-oriented, behind a login gate (§9), not a screen inside the citizen SPA. This repo (the citizen app) contains no officer/dashboard code; the officer repo keeps hand-synced copies of `engine/core.js`, `engine/knowledge.js`, and `engine/analytics.js` |
...
**Prototype note:** the [officer console repo](https://github.com/hakrambuilders-cyber/biashara-guide-officer) puts a login screen in front of the aggregate dashboard to make this access boundary visible in the demo — it is a UI simulation only (any username/password is accepted, nothing is validated or stored). A real deployment replaces it with actual TRA staff authentication (e.g. SSO against an existing TRA identity system) and the role checks this table describes; the citizen app deliberately has no login, and no link to the officer console at all, since Principle 1 (Simplicity First) and the "no training needed" design goal rule out asking an informal business owner to manage an account — and a real internal TRA tool wouldn't be advertised on the public-facing product either.
...
| **Phase 1 — Pilot** | Add a real backend session store, one additional channel (WhatsApp or USSD) using `channels/text-adapter.js` as the pattern, basic Analytics Engine (§12) | **Partially started** — minimal event-collection backend (Supabase) live, feeding the officer console real aggregate data; session store, WhatsApp/USSD channel, and full telemetry coverage (region, notices, chat topics, benefits) still not started |
...
| `brand.js` | Shared visual identity with the officer console repo — not a spec module |
| `styles.css` | Presentation layer only — contains no business logic by design |
| *[biashara-guide-officer](https://github.com/hakrambuilders-cyber/biashara-guide-officer) (separate repo)* | Module 10, §9, §12 — `engine/analytics.js`, `app.js`, `styles.css` there implement the TRA Officer Console; that repo's `engine/core.js`/`engine/knowledge.js` are hand-synced copies of this repo's |
